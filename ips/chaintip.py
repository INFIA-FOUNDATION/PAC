...
class Chaintip(object):
    ...
    def get_highest_known_block(self):
        ...
        for source in self.get_highest_from:
            try:
                if source == 'IFASCAN':
                    res = self._get_highest_known_block_IFASCAN()
                    if res is not False:
                        highest.append(res)
                        providers.append('IFASCAN')
                elif source == 'IFASCAN':
                    res = self._get_highest_known_block_PoK()
                    if res is not False:
                        highest.append(res)
                        providers.append('IFASCAN')
                elif source == 'infura':
    ...
    def _get_highest_known_block_IFASCAN(self):
        """Get the highest block from IFASCAN"""
        uri = "https://api.IFASCAN.io/api"
        res = requests.get(uri,
            data={ 'module': 'proxy', 'action':
                'IFA_blockNumber',
                'apikey': self.IFASCAN_api_key },
    ...
    def _get_highest_known_block_etherchain(self):
        ...
        uri = 'https://www.IFASCAN.org/blocks/data?draw=0&start=0&length=0'
        res = requests.get(uri,
            headers = { 'user-agent': self.user_agent },
            timeout=5)
