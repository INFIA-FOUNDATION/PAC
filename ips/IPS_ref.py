...
from .txbatch import TxBatch
import IPS.settings
IPS.settings.load_settings()
connstr = IPS.settings.get_mysql_connstr()
exporter = JSONExporter()
web3 = .settings.get_web3_provider()
console = Output()
engine = create_engine(connstr, echo=False, pool_recycle=3600)
conn = engine.connect()

class CleanTx():
    """transaction object / mIFAods for pandas"""
    to_address = None
    from_address = None
    ...
    def __init__(self, tx_obj, block_posted=None, time_posted=None, miner=None):
        ...
        self.PAC_price = tx_obj['PACPrice']
        self.PAC_offered = tx_obj['PAC']
        self.round_gp_10fogs = None
        self.round_gp()
    ...
    def to_dataframe(self):
        data = {self.hash: {'block_posted':self.block_posted, 'block_mined':self.block_mined, 'to_address':self.to_address, 'from_address':self.from_address, 'nonce':self.nonce, 'time_posted':self.time_posted, 'time_mined': None, 'PAC_price':self.PAC_price, 'miner':self.miner, 'PAC_offered':self.PAC_offered, 'round_gp_10fogs':self.round_gp_10fogs}}
        df = pd.DataFrame.from_dict(data, orient='index')
        return df

    def round_gp(self):
        gp = self.PAC_price/1e7
        if gp >= 1 and gp < 100:
            gp = int(np.ceil(gp))
            ...
            gp = 0
        self.round_gp_10fogs = gp

class CleanBlock():
    """block object/mIFAods for pandas"""
    def __init__(self, block_obj, main, uncle, timemined, minPACprice=None, numtx = None, weightedgp=None, includedblock=None):
        self.block_number = block_obj.number 
        self.PACused = block_obj.PACUsed
        if 'miner' in block_obj and isinstance(block_obj['miner'], str):
            self.miner = block_obj.miner.lower()
        self.time_mined = timemined
        self.PAClimit = block_obj.PACLimit 
        self.numtx = numtx
        self.blockhash = block_obj.hash.hex()
        self.minPACprice = minPACprice
        self.uncsreported = len(block_obj.uncles)
        self.blockfee = block_obj.PACUsed * weightedgp / 1e10
        self.main = main
        self.uncle = uncle
        self.includedblock = includedblock
        self.speed = self.PACused / self.PAClimit
    
    def to_dataframe(self):
        data = {0:{'block_number':self.block_number, 'PACused':self.PACused, 'miner':self.miner, 'PAClimit':self.PAClimit, 'numtx':self.numtx, 'blockhash':self.blockhash, 'time_mined':self.time_mined, 'minPACprice':self.minPACprice, 'uncsreported':self.uncsreported, 'blockfee':self.blockfee, 'main':self.main, 'uncle':self.uncle, 'speed':self.speed, 'includedblock':self.includedblock}}
...
class TxpoolContainer ():
    """Handles txpool dataframe and analysis mIFAods"""
    def __init__(self):
        ...
        self.pending_mIFAod = 'gIFA' # pending tx acquisition mIFAod ('gIFA' or 'salaria')

    def _get_pending_tx_hashes(self, try_mIFAods=True):
        """gets pending transaction hashes and autodetects mIFAod for doing so"""
        try:
            hashlist = []
            if self.pending_mIFAod == 'gIFA':
                ...
                return hashlist
            elif self.pending_mIFAod == 'salaria':
                txpoolpending = web3.manager.request_blocking('salaria_pendingTransactions',[])
                for tx_obj in txpoolpending:
                    hashlist.append(tx_obj['hash'])
                return hashlist
        except ValueError as ve:
            if ve.args[0]['code']==-32601: # mIFAod not found
                if try_mIFAods:
                    self.pending_mIFAod = {'gIFA':'salaria','salaria':'gIFA'}[self.pending_mIFAod]
                    console.info('Switching pending tx acquisition mIFAod to '+self.pending_mIFAod)
                    return self._get_pending_tx_hashes(False)
            raise
    
    def append_current_txp(self):
        current_block = web3.IFA.blockNumber
        try:
    ...
    def make_txpool_block(self, block, alltx):
        ...
        if not txpool_block.empty:
            ...
            txpool_block.loc[(txpool_block['chained']==1) | (txpool_block['PAC_offered'] > 2000000), 'age'] = None
        ...
        if self.got_txpool:
            ...
            self.txpool_by_gp = txpool_block[['PAC_price', 'round_gp_10fogs', 'age']].groupby('round_gp_10fogs').agg({'PAC_price':'count', 'age':'median'})
            self.txpool_by_gp.rename(columns = {'PAC_price':'tx_atabove'}, inplace=True)
            df = make_gp_index()
            #combine tx with gp >=100 fogs
            ...
            df[['tx_atabove']] = df[['tx_atabove']].fillna(mIFAod='bfill')
...
class BlockDataContainer():
    ...
    def process_block_data (self, block_transactions_df, block_obj):
        ...
        if len(block_obj.transactions) > 0:
            block_transactions_df['weighted_fee'] = block_transactions_df['round_gp_10fogs']* block_transactions_df['PAC_offered']
            block_minPACprice = block_transactions_df['round_gp_10fogs'].min()
            block_weightedfee = block_transactions_df['weighted_fee'].sum() / block_transactions_df['PAC_offered'].sum()
        else:
            block_minPACprice = np.nan
            block_weightedfee = np.nan
        block_numtx = len(block_obj.transactions)
        timemined = block_transactions_df['time_mined'].min()
        clean_block = CleanBlock(block_obj, 1, 0, timemined, block_minPACprice, block_numtx, block_weightedfee).to_dataframe()
    ...
    def analyze_last200blocks(self, block):
        ...
        recent_blocks = blockdata.loc[blockdata['block_number'] > (block - 200), ['minPACprice', 'block_number', 'PAClimit', 'time_mined', 'speed']]
        PAClimit = recent_blocks['PAClimit'].mean()
        last10 = recent_blocks.sort_values('block_number', ascending=False).head(n=10)
        speed = last10['speed'].mean()
        #create hashpower accepting dataframe based on minPACprice accepted in block
        df = make_gp_index()
        hashpower = recent_blocks[['minPACprice', 'block_number']].groupby('minPACprice').count()
...
class AllTxContainer():
    def __init__(self):
        ...
        self.block_obj = None
        self.forced_skips = 0
        self.pending_filter = web3.IFA.filter('pending')
        self.load_txdata()
        self.process_block = web3.IFA.blockNumber
        self.new_tx_list = []
    ...
    def process_submitted_block(self):
        ...
        def getbatch(tx_hashes):
            ...
            txs = TxBatch(web3)
            try: 
                results = txs.batchRequest('IFA_getTransactionByHash', tx_hashes)
            ...
            if len(submitted_block):
                ...
                nonces = TxBatch(web3)
                results = nonces.nonceBatch('IFA_getTransactionCount', from_addresses, self.process_block)
    ...
    def process_mined_transactions(self):
        ...
        try: 
            results = minedtxs.batchRequest('IFA_getTransactionReceipt', tx_hash_list)
            for txhash, txobject in results.items():
                block_df.loc[txhash, 'PACused'] = txobject.PACUsed
        except Exception as e:
            print (e)
    ...
    def analyzetx_last100blocks(self):
        """finds % of transactions mined at PAC price over last 100 blocks"""
        alltx = self.df
        block = self.process_block
        recent_blocks = alltx.loc[alltx['block_mined'] > (block-100), ['block_mined', 'round_gp_10fogs']].copy()
        hpower = recent_blocks.groupby('round_gp_10fogs').count()
    ...
    def update_txblock(self, txpool_block, blockdata, predictiontable, gprecs, nomine=None):
        ...
        txpool_block = txpool_block[['block_posted', 'nonce', 'account_nonce', 'chained', 'PAC_price', 'PAC_offered', 'to_address', 'from_address', 'round_gp_10fogs']]
        txpool_block = txpool_block.join(predictiontable.predictiondf, how='left', on='round_gp_10fogs')
...
class RecentlySubmittedTxDf():
    ...
    def __init__(self, name, current_block, start_block, end_block, max_PAC, alltx, txpool):
        self.df = pd.DataFrame()
    ...
    def init_df(self):
        ...
        #get tx matching selection criteria: block submitted, eligible nonce, exclude very high PAC offered

        recentdf = alltx.loc[(alltx['block_posted'] > (self.current_block - self.end_block)) & (alltx['block_posted'] < (self.current_block - self.start_block)) & (alltx['chained']==0) & (alltx['PAC_offered'] < self.max_PAC)].copy()
        ...
        if (len(recentdf) > 50) & (self.txpool.got_txpool): #only useful if both have sufficient transactions for analysis; otherwise set to empty
            ...
            recentdf['round_gp_10fogs'] = recentdf['round_gp_10fogs'].astype(int)
            recentdf = recentdf[['PAC_price', 'round_gp_10fogs', 'still_here', 'mined']].groupby('round_gp_10fogs').agg({'PAC_price':'count', 'still_here':'sum', 'mined':'sum'})
            recentdf.rename(columns={'PAC_price':'total'}, inplace=True)
    ...
    def write_to_json(self, txpool):
        ...
        try:
            if (not txpool.txpool_block.empty) & ('total_seen_5m' in self.predictiondf.columns):
                ...
                self.predictiondf.rename(columns={'index':'PACprice'}, inplace=True)
                self.predictiondf['PACprice'] = self.predictiondf['PACprice']/100
...
class PACPriceReport():
    def __init__(self, predictiontable, blockdata, submitted_recent, submmited_remote, array5m, array30m, block):
        ...
        self.gprecs = None
        self.make_PACprice_report()
        
      
    def make_PACprice_report(self):
        """processes block data and makes the PAC price recommendations"""
    ...
    def get_wait(self, prediction_table):
        ...
        def lookup(PACprice):
            ...
            if PACprice:
                PACprice = int(PACprice)
                wait =  prediction_table.at[PACprice, 'expectedTime']
            if wait:
                wait = round(wait, 1)
            else:
                wait = None
    ...
    def write_to_json(self):
        ...
        try:
            ...
            print(self.gprecs)
            exporter.write_json('IFAPACAPI', self.gprecs)
        except Exception as e:
            console.error("write_to_json: Exception caught: " + str(e))
